...
class Trivia(commands.Cog):
    ...
    async def trivia(self, ctx, num_questions: int = 1, *, args: str = None):
        ...
        if ctx.channel in self.ongoing_games:
            await ctx.send(
                f"There is already a trivia game in progress in #{ctx.channel}"
            )
            return
    ...
    async def _start_game(self, channel, num_questions, hangman=False):
        try:
            ...
            await channel.send(
                f"Error encountered while fetching questions: {type(e).__name__}"
            )
        ...
        for question in questions:
            ...
            hint_task = None
            if hangman:
                hint_task = self.bot.loop.create_task(
                    self._hint_task_v2(channel, question["correct_answer"])
                )
            else:
            ...
            while True:
                ...
                try:
                    guess = await self.bot.wait_for(
                        "message", timeout=60.0, check=guess_check
                    )
                ...
                if timer() - time_asked > 60:
                    await channel.send(
                        f"Time's up! The answer was: {question['correct_answer']}"
                    )
                    if hangman:
                        hint_task.cancel()
                    break
                ...
                if (
                    not hangman
                    and guess.content.strip().startswith("#")
                    and guess.content.strip()[1].isdigit()
                ):
                    guess_num = int(guess.content.strip()[1])
                    if (
                        1 <= guess_num <= 4
                        and choices[guess_num - 1] == question["correct_answer"]
                    ):
    ...
    async def _hint_task(self, channel, answer):
        ...
        for i in range(3):
            ...
            await channel.send(
                f"Hint: {hint_string} T+{str(int(timer() - start_time))}s"
            )
    ...
    async def _hint_task_v2(self, channel, answer):
        ...
        answer = answer.strip()
        hint_reveal_pool = [
            i for i in range(len(answer)) if answer[i].isdigit() or answer[i].isalpha()
        ]
        hint_string_source = [
            "\_ " if elem.isdigit() or elem.isalpha() else elem.replace(" ", "  ")
            for elem in answer
        ]
        reveal_per_hint = int(len(answer) * 0.25)
        start_time = timer()
    ...
    async def _get_questions(
        self, num_questions=10, category=None, multiple_choice=True
    ):
        params = {"amount": num_questions, "type": "multiple", "encoding": "base64"}

        if not self.token or (
            self.last_response_time and (timer() - self.last_response_time >= 21600)
        ):
        ...
        async with self.bot.http_session.get(
            ...
            for question in json_response["results"]:
                ...
                for i in range(len(question["incorrect_answers"])):
                    question["incorrect_answers"][i] = unescape(
