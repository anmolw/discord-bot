...
class RocketLeague(commands.Cog):
    ...
    async def rl(self, ctx, reference: str = None):
        ...
        if not reference:
            await self.bot.say(
                f"{ctx.message.author.mention} Please specify steamid64 as a parameter"
            )
            return
        ...
        async with ctx.channel.typing():
            async with self.bot.http_session.get(
                ...
                image_response = await self.bot.http_session.get(
                    json_response["signatureUrl"]
                )
                image = discord.File(
                    fp=io.BytesIO(await image_response.read()), filename="stats.png"
                )
